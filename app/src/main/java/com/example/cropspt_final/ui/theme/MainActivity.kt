...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun runObjectDetection(bitmap: Bitmap) {
        ...
        if (hasLettuce) {
            ...
            startResultsActivity(resultToDisplay)
            val processedResults = getDetectionResultsWithArea(resultToDisplay)
            var spoilagePercentage = 0

            // Access the processed results (list of DetectionInfo objects)
            processedResults.forEach { info ->
                Log.i(
                    TAG, "Class: ${info.className}, Area: ${info.area}, Percent: ${info.percent}, Scale: ${info.scale}"
                )
                spoilagePercentage += info.percent
                Log.i(TAG, "Spoilage Percentage: $spoilagePercentage")
            }
    ...
    fun startResultsActivity(resultToDisplay: List<DetectionResult>, spoilagePercentage: Int = 0) {
        val classNames = resultToDisplay.map { it.text.split(",").first().trim() }
        val intent = Intent(this, ResultsScreen::class.java).apply {
            putStringArrayListExtra("classNames", ArrayList(classNames))
            putExtra("spoilagePercentage", spoilagePercentage)
        }
    ...
    fun getDetectionResultsWithArea(
        detectionResults: List<DetectionResult>
    ): List<DetectionInfo> {
    ...
    private fun listSpoilageScales(processedResults: List<DetectionInfo>): List<SpoilageScales> {
        val results: Array<SpoilageScales>? = intent.getParcelableArrayExtra("results") as? Array<SpoilageScales>
